...
const previous = issues.find((entry) => entry.date === "2026-08-23");
if (!previous) throw new Error("Missing 2026-08-23 source issue");

const date = "2026-08-24";
const issue = structuredClone(previous);
issue.date = date;
issue.zhTitle = "AI Daily 2026-08-24：Agentic AI 开始按观察链路重做计算栈";
issue.enTitle = "AI Daily 2026-08-24: Agentic AI starts rebuilding the compute stack around observation loops";
issue.zhSummary = "今日最强的产品硬件信号来自 Intel Hot Chips 2026：Diamond Rapids、Crescent Island、Wildcat Lake 把 agentic AI 从机架推向数据中心推理、客户端和边缘设备。与此同时，雷鸟 iO 用无摄像头显示做日常佩戴，Razer Motoko 把双目视觉放入耳罩，Orbbec 与 51WORLD 把第一视角采集、同步、质检和训练数据做成具身基础设施。今天的产品问题因此变得具体：哪个设备负责观察，哪个芯片负责推理，用户何时能看到系统正在看，数据如何进入下一步动作。";
issue.enSummary = "Today’s strongest hardware signal comes from Intel at Hot Chips 2026: Diamond Rapids, Crescent Island, and Wildcat Lake map agentic AI from the rack to datacenter inference, clients, and edge devices. At the same time, RayNeo iO makes a camera-free display wearable, Razer Motoko puts stereo vision in an earcup, and Orbbec plus 51WORLD package first-person capture, synchronization, QA, and training data as embodied-AI infrastructure. The product question becomes concrete: which device observes, which silicon infers, when can users see that the system is looking, and how does evidence become the next action?";
...
for (const topic of issue.topics) topic.sourceDate = `${topic.sourceDate} · 2026-08-24 current source sweep`;
