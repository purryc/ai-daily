...
issue.date = date;
issue.zhTitle = "AI Daily 2026-09-17：广告 agent、独立 AR 与百万级算力同时进入产品面";
issue.enTitle = "AI Daily 2026-09-17: Sponsored agents, standalone AR, and million-scale compute enter the product surface";
issue.zhSummary = "OpenAI 把 Sponsored Agent 放进广告点击后的对话路径，Snap SPECS 将 AI、显示、手势和蜂窝连接装进独立 AR 眼镜；华为用 Atlas 960E、Peerium 与 HarmonyOS Agent OS 路线把 agent 的算力和跨设备边界推向系统层。昨日的本地 agent 与 physical AI 继续保留，研究、社区、专利和野生 lane 仍明确标注证据强弱。";
issue.enSummary = "OpenAI puts a Sponsored Agent inside the post-ad conversation path, while Snap SPECS puts AI, display, hand tracking, and cellular connectivity into standalone AR glasses. Huawei pushes agent compute and cross-device boundaries into the system layer through Atlas 960E, Peerium, and its HarmonyOS Agent OS direction. Yesterday's local-agent and physical-AI items remain, with research, community, patent, and wild lanes explicitly downgraded where needed.";
issue.tags = Array.from(new Set(["Sponsored Agents", "ChatGPT Ads", "standalone AR", "Snap SPECS", "Huawei Atlas 960E", "Peerium", "Agent OS", "cross-device AI", "HCI", "trust", ...issue.tags])).slice(0, 36);
...
issue.topics = [...freshTopics, ...issue.topics.filter((item) => !item.id.startsWith("snap-specs"))];
issue.coverStory = {
  topicId: freshTopics[0].id,
  zhTitle: "广告、眼镜和算力：AI 正在把‘边界’变成产品界面",
  enTitle: "Ads, glasses, and compute: AI is turning boundaries into product interfaces",
  zhSummary: ["Sponsored Agents 把商业身份和对话上下文隔离摆到广告点击之后。", "Snap SPECS 把显示、手势和主动浮现带到独立眼镜，但公共空间里的隐私与失败成本更高。", "华为的 Atlas 960E / Peerium 说明长任务 agent 的体验上限也由互联、内存、故障和成本决定。"],
  enSummary: ["Sponsored Agents put commercial identity and context separation after the ad click.", "Snap SPECS brings display, hand input, and proactive surfacing into standalone glasses, raising privacy and failure costs in public space.", "Huawei's Atlas 960E and Peerium show that long-running agent UX is bounded by interconnect, memory, failure, and cost."],
  imagePath: openaiVisual.path, imageWidth: openaiVisual.width, imageHeight: openaiVisual.height, imageSourceUrl: openaiVisual.sourceUrl, primarySourceUrl: openaiUrl,
  evidenceStrength: "confirmed product · OpenAI official test / 2026-09-16; paired with review and infrastructure evidence",
  whyCover: "The decisive product work is making identity, context, execution location, and takeover visible before an agent acts."
};
issue.designDesk = {
  zhTitle: "Design Desk：先画清楚 agent 的边界，再谈自然交互",
  enTitle: "Design Desk: draw the agent boundary before making interaction feel natural",
  zhIntro: "今天的新增产品把商业身份放进对话，把主动信息放进视野，把百万级资源放进系统。它们共同要求设计团队把边界变成一等界面：谁在说话、用了什么上下文、在哪台设备执行、会消耗什么资源，以及用户怎样接管。",
  enIntro: "Today's additions put commercial identity into chat, proactive information into view, and million-scale resources into the system. Together they require teams to make boundaries first-class UI: who is speaking, which context was used, where execution happens, what resources are consumed, and how the user takes over.",
  zhItems: [
...
issue.watchlistZh = [
  ...
  "Snap SPECS：价格与发货、4 小时续航、51 度视场角、摄像头/录制状态、公共空间接受度、Lens SDK。",
  "Huawei Atlas 960E / Peerium：独立 benchmark、客户部署、统一内存开发模型、故障恢复、能耗和租户隔离。",
  "Huawei Agent OS：跨设备 task ID、权限继承、离线 fallback、车/家高风险动作确认和暂停语义。",
...
issue.watchlistEn = [
  ...
  "Snap SPECS: price and shipping, four-hour battery claim, 51-degree FOV, camera/recording state, public acceptance, and Lens SDK.",
  "Huawei Atlas 960E / Peerium: independent benchmarks, customer deployment, unified-memory programming, recovery, energy, and tenant isolation.",
  "Huawei Agent OS: cross-device task ID, permission inheritance, offline fallback, confirmation for vehicle/home actions, and pause semantics.",
...
for (const file of ["openai-sponsored-agents-research-2026-09.png", "snap-specs-review-2026-09.png", "huawei-atlas-960e-2026-09.png", "huawei-peerium-2026-09.png"]) {
